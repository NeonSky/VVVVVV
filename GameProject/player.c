...
void movePlayer() {
  ...
  if(player.isAirborne) {
    ...
    // Ground movement
    if(leftBtn) {
      if(isTraversable(player.x-1, player.y)) { player.x--; }
    }
    else if(rightBtn) {
      if(isTraversable(player.x+1, player.y)) { player.x++; }
    }
...
void clampPlayerPos() {
  ...
  else if(player.y > lcdHeight*2) { player.y = lcdHeight*2; }
...
void updatePlayerSprite() {
  ...
  if(player.y%2 == 0) {
    if(player.isFaceUp) { playerSprite = 1; }
    else { playerSprite = 3; }
  } else {
    if(player.isFaceUp) { playerSprite = 0; }
    else { playerSprite = 2; }
  }
